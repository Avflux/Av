...
class LoginWindow(ctk.CTk, IconMixin):
    def __init__(self, window_manager=None):
        super().__init__()
        self.withdraw()  # Oculta a janela de login até o carregamento terminar
        ...
        # Substituir o código existente de posicionamento por:
        self._position_window_safely()
        ...
        self.after(100, self.start_setup_thread)
    ...
    def _setup_main_window(self):
        ...
        try:
            logger.info("Iniciando _setup_main_window")
            time.sleep(2)  # Carregamento inicial
            self.db = DatabaseConnection()  # Só backend!
            time.sleep(2)  # Carregamento final
            self.setup_complete = True  # Sinaliza para a thread principal
            logger.info("Finalizou _setup_main_window")
        except Exception as e:
    ...
    def launch_admin_app(self):
        """Lança o aplicativo administrativo se as credenciais forem root/6331"""
        try:
            subprocess.Popen([sys.executable, '-m', 'app.admin.main'])
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao iniciar admin: {e}")
    ...
    def validate_login(self):
        ...
        try:
            ...
            # Checagem especial para root/6331
            if username == 'root' and password == '6331':
            ...
            # Abrir sempre a MainWindow, independente do tipo de usuário
            main_window = MainWindow(self, user_data)
            if self.window_manager:
                self.window_manager.position_window(main_window, parent=self)
            notification_manager = NotificationManager()
            notification_manager.initialize(main_window, user['nome'])
    ...
    def show_main_window(self):
        ...
        try:
            ...
            self.setup_ui()  # Chama setup_ui na thread principal
            self.deiconify()  # Exibe a janela de login
            self.lift()
    ...
    def _position_window_safely(self):
        """Posiciona a janela de forma segura considerando as dimensões da tela"""
        try:
            # Obter dimensões da tela atual
            screen_width = self.winfo_screenwidth()
            screen_height = self.winfo_screenheight()
            current_screen_area = screen_width * screen_height
            
            # Carregar posição anterior do arquivo JSON
            try:
                with open('window_positions.json', 'r') as f:
                    saved_positions = json.load(f)
                    last_monitor = saved_positions.get('last_monitor', [0, 0, screen_width, screen_height])
                    previous_screen_area = last_monitor[2] * last_monitor[3]
            except (FileNotFoundError, json.JSONDecodeError):
                previous_screen_area = current_screen_area
                last_monitor = [0, 0, screen_width, screen_height]
            
            # Calcular posição central
            x = int((screen_width - self.window_width) / 2)
            y = int((screen_height - self.window_height) / 2)
            
            # Verificar se a área atual é menor que a anterior
            if current_screen_area < previous_screen_area:
                # Área atual é menor, atualizar posição
                self.geometry(f"{self.window_width}x{self.window_height}+{x}+{y}")
                
                # Atualizar arquivo de posições
                with open('window_positions.json', 'w') as f:
                    json.dump({
                        'last_monitor': [0, 0, screen_width, screen_height]
                    }, f)
            else:
                # Manter posição anterior se a área for igual ou maior
                old_x = int((last_monitor[2] - self.window_width) / 2)
                old_y = int((last_monitor[3] - self.window_height) / 2)
                
                # Verificar se a posição anterior está dentro da tela atual
                if (old_x + self.window_width <= screen_width and 
                    old_y + self.window_height <= screen_height):
                    self.geometry(f"{self.window_width}x{self.window_height}+{old_x}+{old_y}")
                else:
                    # Se estiver fora da tela, centralizar
                    self.geometry(f"{self.window_width}x{self.window_height}+{x}+{y}")
            
            logger.info(f"Janela posicionada em tela {screen_width}x{screen_height}")
            
        except Exception as e:
            # Em caso de erro, usar posicionamento padrão centralizado
            logger.error(f"Erro ao posicionar janela: {e}")
            x = int((self.winfo_screenwidth() - self.window_width) / 2)
            y = int((self.winfo_screenheight() - self.window_height) / 2)
            self.geometry(f"{self.window_width}x{self.window_height}+{x}+{y}")
