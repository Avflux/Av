...
class ActivityControls(ctk.CTkFrame, TimeObserver):
    def __init__(self, parent, user_data, db, on_activity_action,
                 active_label=None, selected_label=None, daily_time_manager=None):
        ...
        # Prioriza o daily_time_manager passado, senão busca na main_window
        if daily_time_manager is not None:
            self.daily_time_manager = daily_time_manager
        else:
    ...
    def update_button_states(self, force_state=None):
        ...
        try:
            ...
            # Verificar se é 10 minutos antes do fim do expediente
            warning_time = (company_end.hour * 60 + company_end.minute - 10)
    ...
    def _create_dialog_safely(self, dialog_type, dialog_class):
        """Cria um diálogo de forma segura, evitando duplicatas"""
        try:
            # Verifica se já está criando este tipo de diálogo
            if self._creating_dialog[dialog_type]:
                return
            
            # Marca que está criando o diálogo
            self._creating_dialog[dialog_type] = True
            
            # Usa self.main_window que já é a referência correta para UserWindow ou AdminWindow
            dialog = dialog_class(parent=self.main_window)
            
            # Atualiza o timestamp de exibição
            self._dialogs_shown_today[dialog_type] = datetime.now().timestamp()
            
            # Armazena referência ao diálogo
            if dialog_type == 'break_start':
                self._break_start_dialog = dialog
            elif dialog_type == 'break_end':
                self._break_end_dialog = dialog
            elif dialog_type == 'company_end':
                self._company_end_dialog = dialog
            elif dialog_type == 'company_end_warning':
                self._company_end_warning_dialog = dialog
            
            # Reseta flag de criação
            self._creating_dialog[dialog_type] = False
            
        except Exception as e:
            logger.error(f"Erro ao criar diálogo {dialog_type}: {e}")
            self._creating_dialog[dialog_type] = False
            ...
            if activity_info and 'id' in activity_info:
                ...
                if result and result[0]['time_exceeded'] != '00:00:00':
                    ...
                    if not self._time_exceeded_dialog or not self._time_exceeded_dialog.winfo_exists():
                        # Usar self.main_window como parent
                        self._time_exceeded_dialog = TimeExceededDialog(self.main_window, activity_info.get('atividade'))
