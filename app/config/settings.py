...
class EncryptedSettings:
    def __init__(self):
        ...
        self.key_file = os.path.join(base_dir, "crypto.key")
        self.env_file = os.path.join(base_dir, ".env.encrypted")
        
        logger.debug(f"Diretório base: {base_dir}")
        logger.debug(f"Caminho do arquivo de chave: {self.key_file}")
        logger.debug(f"Caminho do arquivo env: {self.env_file}")
    ...
    def _get_fernet(self):
        ...
        try:
            ...
            # Tenta procurar no diretório atual como fallback
            current_dir = os.getcwd()
            fallback_key_file = os.path.join(current_dir, "crypto.key")
            
            logger.debug(f"Tentando fallback no diretório atual: {fallback_key_file}")
            
            try:
                with open(fallback_key_file, "rb") as key_file:
                    key = key_file.read()
                # Se encontrou no diretório atual, atualiza o caminho
                self.key_file = fallback_key_file
                return Fernet(key)
            except FileNotFoundError:
                error_msg = "Arquivo de chave criptográfica não encontrado.\nExecute o aplicativo de configuração primeiro."
                logger.error(f"Arquivo de chave não encontrado em: {self.key_file} nem em {fallback_key_file}")
    ...
    def decrypt_env(self):
        ...
        try:
            if not os.path.exists(self.env_file):
                # Tenta procurar no diretório atual como fallback
                current_dir = os.getcwd()
                fallback_env_file = os.path.join(current_dir, ".env.encrypted")
                
                logger.debug(f"Tentando fallback do env no diretório atual: {fallback_env_file}")
                
                if os.path.exists(fallback_env_file):
                    self.env_file = fallback_env_file
                else:
                    error_msg = "Arquivo de configurações criptografadas não encontrado.\nExecute o aplicativo de configuração primeiro."
                    logger.error(f"Arquivo não encontrado em: {self.env_file} nem em {fallback_env_file}")
                    raise FileNotFoundError(error_msg)
